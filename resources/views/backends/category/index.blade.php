...
@section('contents')
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between pb-0">
                            <h5 class="pb-1">{{ __('Categories Table') }}</h5>
                            @if (auth()->user()->can('category.create'))
                                <a class="btn bg-gradient-primary btn-modal add-new-button-right-side btn-xs" href="#" data-href="{{ route('admin.category.create') }}"
                        ...
                        <div class="card-body px-3 pt-0 pb-2">
                            ...
                            @include('backends.category._table')
                        </div>
                    </div>
                </div>
            </div>
    ...
    @include('backends.category.partial.delete_category_modal')
@endsection
@push('js')
    <script>
    ...
    <script>
        ...
        $(document).on('click', '.btn-delete', function (e) {
            ...
            $('#deleteCategoryModal').data('category-id', categoryId).data('delete-url', deleteUrl).modal('show');
        });
        ...
        $(document).on('click', '.btn-confirm-modal', function () {
            ...
            let row = $(`.btn-delete[data-id="${categoryId}"]`).closest('tr');
            let dataTable = $('#bookingTable').DataTable();

            var data = $(`.form-delete-${categoryId}`).serialize();
        ...
        //for update status
        initializeStatusInput("{{ route('admin.category.update_status') }}");
    </script>
@endpush
